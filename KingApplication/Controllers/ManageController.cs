    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                message == ManageMessageId.ChangePseudoSuccess ? "Votre pseudo a été changé."
        ...
        //
        // GET: /Manage/ChangePseudo
        public ActionResult ChangePseudo()
        {
            return View();
        }

        //
        // POST: /Manage/ChangePseudo
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ChangePseudo(ChangePseudoViewModel model)
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            ...
            var user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
            user.UserName = model.NewPseudo;
            var result = await UserManager.UpdateAsync(user);
            if (result.Succeeded)
            {
                return RedirectToAction("Index", new { Message = ManageMessageId.ChangePseudoSuccess });
            }
            AddErrors(result);
            return View(model);
        }
